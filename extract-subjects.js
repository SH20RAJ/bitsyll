...
subjectFolders.forEach(folder => {
  ...
  const codeMatch = content.match(/\(([A-Z]{2}\d{5})\)/);
  const titleMatch = content.match(/\(([A-Z]{2}\d{5})\)[^<]*<br \/>[^<]*([^<]+)/);
  
  if (!codeMatch || !titleMatch) {
    console.log(`Skipping ${folder}: Could not extract code or title`);
    return;
  }
  ...
  if (!syllabusMatch) {
    console.log(`Warning: Could not extract syllabus for ${folder}`);
  }
  ...
  const subject = {
    ...
    semester: parseInt(code.charAt(3)), // Extract semester from code
    department,
